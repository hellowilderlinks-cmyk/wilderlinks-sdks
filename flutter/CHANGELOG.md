...
- Initial release.
- `WilderlinksSdk.handleIncomingUri` — resolve a Universal Link / App Link when the app is already installed.
- `WilderlinksSdk.checkDeferredInstall` / `matchDeferredToken` — deferred deep link matching after a fresh install.
- `WilderlinksListener` — stream-based wrapper around `app_links` that wires the two together automatically.
